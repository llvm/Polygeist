/* Fixed-shape ATen antialiased lanczos 2D backward. */
#ifndef B
#define B 1
#endif
#ifndef C
...
void aten_upsample_lanczos2d_aa_backward_cpu(float grad_output[B*C*O0*O1], float grad_input[B*C*I0*I1]) {
  ...
  for (int oy = 0; oy < O0; ++oy) for (int ox = 0; ox < O1; ++ox) {
    ...
    for (int iy = 0; iy < I0; ++iy) for (int ix = 0; ix < I1; ++ix) {
      ...
      float wy = ay < 3.0f ? (ay == 0.0f ? 1.0f : (ay < 3.0f ? sinf(3.14159265358979323846f * ay) * sinf(3.14159265358979323846f * ay / 3.0f) / (3.289868133696453f * ay * ay) : 0.0f)) : 0.0f;
      float wx = ax < 3.0f ? (ax == 0.0f ? 1.0f : (ax < 3.0f ? sinf(3.14159265358979323846f * ax) * sinf(3.14159265358979323846f * ax / 3.0f) / (3.289868133696453f * ax * ax) : 0.0f)) : 0.0f;
      norm += wy * wx;
    }
    for (int iy = 0; iy < I0; ++iy) for (int ix = 0; ix < I1; ++ix) {
      float ay = sy - (float)iy; if (ay < 0.0f) ay = -ay; ay /= fy_scale;
