...
foreach
  my $arg(@ARGV) {
    ...
    elsif(!($arg = ~ / ^- /)) { $TARGET_DIR = $arg; }
  }

my % categories = ('linear-algebra/blas' = > 3, 'linear-algebra/kernels' = > 3,
                   'linear-algebra/solvers' = > 3, 'datamining' = > 2,
                   'stencils' = > 2, 'medley' = > 2);

my % extra_flags =
    ('cholesky' = > '-lm', 'gramschmidt' = > '-lm', 'correlation' = > '-lm');
...
foreach
  $key(keys % categories) {
	...
	\$ {
      ...
      \${CC} - o $kernel $kernel.c \${CFLAGS} - I.-
          I$utilityDir $utilityDir / polybench.c \${EXTRA_FLAGS}
...
if ($GEN_CONFIG) {
  ...
  print FILE << "EOF";
  CC = gcc CFLAGS = -O2 - DPOLYBENCH_DUMP_ARRAYS -
                    DPOLYBENCH_USE_C99_PROTO EOF
