...
#note that syrk only stores lower triangular part of the resulting symmetric   \
    matrix
}
...
library('Matrix') lu.polybench < -function(N) {
  A < -matrix(0, nrow = N, ncol = N)
...
trisolv < -function(N) {
  L < -matrix(0, nrow = N, ncol = N) b < -c(0
                                            : (N - 1))
