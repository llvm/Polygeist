...
if
  [$ # - ne 1];
then echo "Usage: ./time_benchmarh.sh <binary_name>";
echo "Example: ./time_benchmarh.sh \"./a.out\"";
echo
    "Note: the file must be a Polybench program compiled with -DPOLYBENCH_TIME";
exit 1;
fi;
...
compute_mean_exec_time() {
  ...
  if
    ![-z "$test_err"];
  then echo
      "[ERROR] Program output does not match expected single-line with time.";
  echo
      "[ERROR] The program must be a PolyBench, compiled with -DPOLYBENCH_TIME";
  exit 1;
  fi;
  val12 =`echo "a=$val2 - $time;if(0>a)a*=-1;a" | bc`;
  val13 =`echo "a=$val3 - $time;if(0>a)a*=-1;a" | bc`;
  myvar =`echo "$val11 $val12 $val13" |
         awk '{ if ($1 > $2) { if ($1 > $3) print $1; else print $3; } else { if ($2 > $3) print $2; else print $3; } }'`;
  variance =`echo "scale=5;($myvar/$time)*100" | bc`;
  tmp =`echo "$variance" | cut - d '.' - f 1`;
  if
    [-z "$tmp"];
  then variance = "0$variance";
  fi;
  compvar =`echo "$variance $VARIANCE_ACCEPTED" |
           awk '{ if ($1 < $2) print "ok"; else print "error"; }'`;
  if
    ["$compvar" = "error"];
  then echo
  ...
  else echo "[INFO] Maximal deviation from arithmetic mean of 3 average runs: "
            "$variance%";
  fi;
  PROCESSED_TIME = "$time";
  rm - f avg.out;
...
echo "[INFO] Running 5 times $1..." echo
     "[INFO] Maximal variance authorized on 3 average runs: "
     "$VARIANCE_ACCEPTED%...";
