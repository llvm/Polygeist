...
static void connectPHINodes(
    ...
    const DenseMap<Operation *, llvm::BranchInst *> &branchMapping) {
  ...
  for (auto it = std::next(func.begin()), eit = func.end(); it != eit; ++it) {
    ...
    for (auto &numberedPhiNode : llvm::enumerate(phis)) {
      ...
      for (auto *pred : bb->getPredecessors()) {
        // Find the LLVM IR block that contains the converted branch instruction
        // and use it in the PHI node. Note that this block is not necessarily
        // the same as blockMapping.lookup(pred), some operations (in
        // particular, OpenMP operations using OpenMPIRBuilder) may have split
        // the blocks.
