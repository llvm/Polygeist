  ...
  func.func @kernel_doitgen(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: memref<?x8x12xf64>, %arg4: memref<?x12xf64>, %arg5: memref<?xf64>) attributes {llvm.linkage = #llvm.linkage<external>} {
    ...
    affine.for %arg6 = 0 to %2 {
      affine.for %arg7 = 0 to %0 {
        %3 = polygeist.submap(%arg5, %1) {map = #map} : (memref<?xf64>, index) -> memref<?xf64>
        linalg.generic {indexing_maps = [#map], iterator_types = ["parallel"]} outs(%3 : memref<?xf64>) {
        ^bb0(%out: f64):
          linalg.yield %cst : f64
        }
        %4 = polygeist.submap(%arg3, %arg6, %arg7, %1, %1) {map = #map1} : (memref<?x8x12xf64>, index, index, index, index) -> memref<?x?xf64>
        %5 = polygeist.submap(%arg4, %1, %1) {map = #map2} : (memref<?x12xf64>, index, index) -> memref<?x?xf64>
        %6 = polygeist.submap(%arg5, %1, %1) {map = #map3} : (memref<?xf64>, index, index) -> memref<?x?xf64>
        linalg.generic {indexing_maps = [#map2, #map2, #map2], iterator_types = ["parallel", "reduction"]} ins(%4, %5 : memref<?x?xf64>, memref<?x?xf64>) outs(%6 : memref<?x?xf64>) {
        ^bb0(%in: f64, %in_0: f64, %out: f64):
          %9 = arith.mulf %in, %in_0 : f64
          %10 = arith.addf %out, %9 : f64
          linalg.yield %10 : f64
        }
        %7 = polygeist.submap(%arg5, %1) {map = #map} : (memref<?xf64>, index) -> memref<?xf64>
        %8 = polygeist.submap(%arg3, %arg6, %arg7, %1) {map = #map4} : (memref<?x8x12xf64>, index, index, index) -> memref<?xf64>
