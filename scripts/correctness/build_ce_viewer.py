...
ATEN_PAPER_ANALYSIS_DIR = ATEN_C_ROOT / "paper_analysis"
...
def _aten_campaign_value(row: dict[str, str], phase: str) -> str:
    """Return the headline minimum-of-five value, with legacy fallback."""
    return (row.get(f"{phase}_min_us", "") or
            row.get(f"{phase}_median_us", ""))


def _aten_campaign_phase_ok(row: dict[str, str], phase: str) -> bool:
    """Whether a Section 4.2 phase has an accepted five-sample result."""
    headline = _aten_campaign_value(row, phase)
...
def _aten_campaign_status(kernel: str) -> dict[str, str]:
    ...
    legacy.update({"shape": row.get("shape", ""),
                   "dtype": row.get("dtype", ""),
                   "native_cpu_orin_us": _aten_campaign_value(row, "cpu")})
    native_ok = _aten_campaign_phase_ok(row, "native_gpu")
    raised_ok = _aten_campaign_phase_ok(row, "raised")
    legacy["native_gpu_us"] = (_aten_campaign_value(row, "native_gpu")
                                if native_ok else "")
    legacy["raised_resident_us"] = (_aten_campaign_value(row, "raised")
                                     if raised_ok else "")
    if raised_ok:
        ...
        legacy["status_detail"] = (
            ...
            "samples, minimum reported, device output passed correctness")
    else:
        legacy["raised_status"] = "PENDING_RAISED"
        legacy["status_detail"] = (
            "current Section 4.2 raised resident measurement is pending")
    if native_ok and raised_ok:
        native = float(_aten_campaign_value(row, "native_gpu"))
        raised = float(_aten_campaign_value(row, "raised"))
...
def _aten_slowness_page(aten_stats: dict[str, dict]) -> str:
    ...
    for kernel, campaign in _ATEN_SECTION42.items():
        ...
        try:
            ...
            cpu = float(_aten_campaign_value(campaign, "cpu"))
        except ValueError:
            continue
    ...
    return (
        ...
        'untimed warmups, five synchronized samples, and reports the minimum in '
        ...
        '<th>raised resident (µs, min of 5)</th>'
        '<th>ATen native CUDA resident (µs, min of 5)</th>'
        '<th>ATen CPU, Orin 1 core (µs, min of 5)</th><th>raised/native</th>'
...
def _percent_speedup_chart(
    ...
    """Render a grouped diverging bar chart with explicit missing-result Xs."""
    ...
    rendered_bar_w = max(1.0, bar_w - 1.0)
    for category_index, category in enumerate(categories):
        center = left + (category_index + 0.5) * group_w
        rendered_group_w = ((len(series) - 1) * bar_w + rendered_bar_w)
        start = center - rendered_group_w / 2.0
        for series_index, (label, color, mapping) in enumerate(series):
            ...
            svg.append(
                f'<rect x="{x:.2f}" y="{rect_y:.2f}" width="{rendered_bar_w:.2f}" '
...
def _aten_raised_win_explanation(
        kernel: str, audit: dict[str, str], speedup: float) -> str:
    """Explain a measured win without treating correlation as causal proof.

    This is presentation-only classification from the independently produced
    semantic-family and selected-library fields. It does not participate in
    matching or backend selection.
    """
    family = audit.get("semantic_family", "")
    match = audit.get("current_match", "")
    library = audit.get("candidate_library", "external library")
    if speedup < 1.10:
        return (
            f"Near parity through {library}; the small difference is consistent "
            "with wrapper, dispatch, algorithm-selection, or run-to-run effects "
            "and is not yet an established optimization advantage."
        )
    if family == "sparse_linear_algebra":
        return (
            "The fixed sparse operation reaches cuSPARSE directly, avoiding "
            "some general sparse dispatch and representation handling."
        )
    if family == "data_movement":
        if "unbind" in kernel:
            return (
                "The fixed extracted region becomes one regular 2-D CUDA copy; "
                "the ATen fixture retains multi-output unbind handling."
            )
        return (
            "The fixed geometry lowers to a direct CUDA copy or regular "
            "permutation instead of general tensor construction logic."
        )
    if family in {"tensor_permutation", "patch_extract_scatter"}:
        return (
            "cuTENSOR executes the fixed affine layout transformation as one "
            "coalesced permutation, avoiding general indexing/materialization."
        )
    if family == "tensor_initialization":
        return (
            "The all-zero result lowers to cudaMemset rather than a general "
            "tensor-fill dispatch."
        )
    if family == "dense_vector_update":
        return (
            "The extracted scale is a direct in-place cuBLAS Scal call rather "
            "than a general pointwise multiply."
        )
    if family == "normalization":
        return (
            "A vendor operation graph evaluates the fixed normalization/scale "
            "formula with less general dispatch and intermediate handling."
        )
    if family == "pointwise_reduction_formula":
        return (
            "The fixed pointwise-plus-reduction formula is represented by a "
            "vendor graph rather than separate general ATen stages."
        )
    if family in {"pointwise", "pointwise_formula", "pointwise_math"}:
        if "round" in kernel:
            return (
                "The fixed rounding expression is executed by a vendor "
                "pointwise graph; the measured ATen route has higher arithmetic "
                "or dispatch cost at this shape."
            )
        return (
            "The fixed expression reaches a vendor pointwise operation or graph "
            "with less general TensorIterator/dispatch overhead."
        )
    if family == "convolution":
        return (
            "The fixed convolution maps directly to cuDNN; the modest difference "
            "is most plausibly plan/algorithm and wrapper selection."
        )
    if family == "pooling":
        return (
            "The fixed pooling geometry maps directly to cuDNN Resample; the "
            "small difference is effectively near parity."
        )
    if family == "dense_linear_algebra":
        return (
            "Both paths ultimately use dense-library machinery; the measured "
            "difference is near parity rather than a distinct algorithmic win."
        )
    return (
        f"The fixed region reaches {library} through {match or 'one external call'}, "
        "reducing general framework dispatch for this benchmark shape."
    )


def _aten_section42_paper_page() -> str:
    """Render the active minimum-of-five campaign."""
    ...
    three_way = sum(_aten_campaign_phase_ok(row, "cpu") and
    ...
    framework_pairs = sum(
        row.get("comparability", "").startswith("FRAMEWORK_LEVEL") and
        _aten_campaign_phase_ok(row, "native_gpu") and
        _aten_campaign_phase_ok(row, "raised") for row in rows)
    static_library = sum(
    ...
    coverage_summary = _read_csv(
        ATEN_PAPER_ANALYSIS_DIR / "aten_coverage_summary.csv")
    coverage_rows = [
        '<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(
            html.escape(row.get("section", "")),
            html.escape(row.get("metric", "")),
            html.escape(row.get("kernel_count", "")),
            html.escape(row.get("definition", "")))
    ...
    for row in rows:
        ...
        if row.get("comparability", "").startswith("FRAMEWORK_LEVEL"):
            continue
        native_us = float(_aten_campaign_value(row, "native_gpu"))
        raised_us = float(_aten_campaign_value(row, "raised"))
        cpu_value = _aten_campaign_value(row, "cpu")
        cpu_us = float(cpu_value) if cpu_value else None
        measurements.append({
            "kernel": row["kernel"],
            ...
            "cpu_speedup": cpu_us / raised_us if cpu_us is not None else None,
    ...
    ratio_svg = [
        ...
        'role="img" aria-label="Raised divided by native ATen CUDA minimum-of-five runtime">'
    ]
    ...
    ratio_svg.append(
        ...
        f'{len(measurements)} legally comparable kernels sorted by raised/native ratio; synchronized wall time '
    ...
    for row in rows:
        if not (_aten_campaign_phase_ok(row, "cpu") and
                _aten_campaign_phase_ok(row, "raised")):
            continue
        cpu_us = float(_aten_campaign_value(row, "cpu"))
        raised_us = float(_aten_campaign_value(row, "raised"))
    ...
    cpu_ratio_svg = [
        f'<svg class="paper-chart" viewBox="0 0 {chart_width} {chart_height}" '
        'role="img" aria-label="Orin one-core CPU divided by raised GPU minimum-of-five runtime">'
    ]
    ...
    absolute_svg = [
        f'<svg class="paper-chart" viewBox="0 0 {chart_width} {chart_height}" '
        'role="img" aria-label="ATen CUDA and raised resident minimum-of-five runtime in microseconds">'
    ]
    ...
    absolute_svg.append(
        f'<text class="paper-axis-label" x="{chart_width/2:.1f}" '
        f'y="{chart_height-10}" text-anchor="middle">'
        'same kernel order; minimum-of-five synchronized resident runtime (µs)</text></svg>')

    data_rows = []
    for rank, item in enumerate(measurements, 1):
        cpu_text = (f'{item["cpu"]:,.3f}'
                    if item["cpu"] is not None else "—")
        gpu_percent = _percent_speedup(item["native"], item["raised"])
        cpu_percent = (_percent_speedup(item["cpu"], item["raised"])
                       if item["cpu"] is not None else None)
        data_rows.append(
            '<tr>'
            ...
            f'<td>{cpu_text}</td><td>{item["native"]:,.3f}</td>'
            f'<td>{item["raised"]:,.3f}</td><td>{_format_percent(gpu_percent)}</td>'
            f'<td>{_format_percent(cpu_percent)}</td></tr>')

    competitive_rows = []
    competitive = sorted(
        (item for item in measurements if item["ratio"] < 1.6),
        key=lambda item: (item["ratio"], item["kernel"]),
    )
    raised_winners = sum(item["ratio"] < 1.0 for item in competitive)
    near_parity = sum(0.9 <= item["ratio"] < 1.1 for item in competitive)
    for item in competitive:
        audit = library.get(item["kernel"], {})
        native_over_raised = 1.0 / item["ratio"]
        route = " / ".join(filter(None, (
            audit.get("candidate_library", ""),
            audit.get("current_match", ""),
        ))) or "external route not recorded"
        interpretation = (
            "raised GPU faster"
            if item["ratio"] < 0.9
            else "competitive parity (within 10%)"
            if item["ratio"] < 1.1
            else "competitive; raised GPU within 1.6x of native"
        )
        likely_reason = (
            _aten_raised_win_explanation(
                item["kernel"], audit, native_over_raised)
            if item["ratio"] < 1.0
            else "The fixed library mapping is competitive with native ATen; "
                 "the remaining gap is consistent with wrapper, plan, layout, "
                 "or synchronization overhead."
        )
        competitive_rows.append(
            '<tr>'
            f'<td><code>{html.escape(item["kernel"])}</code></td>'
            f'<td>{item["native"]:,.3f}</td><td>{item["raised"]:,.3f}</td>'
            f'<td><b>{item["ratio"]:.3f}&times;</b></td>'
            f'<td><code>{html.escape(route)}</code></td>'
            f'<td>{html.escape(likely_reason)}</td>'
            f'<td>{html.escape(item["comparability"].replace("_", " ").lower())}</td>'
    ...
    competitive_figure = (
        ...
        'ATen competitive kernels: raised GPU vs native GPU runtime</h4>'
        f'<object data="{figure_url}#view=FitH" type="application/pdf" '
        'width="100%" height="720" '
        'aria-label="ATen raised GPU versus native GPU runtime">'
        f'<p><a href="{figure_url}">Open the exact paper PDF</a></p>'
    ...
    library_summary_rows = [
        '<tr><td>{}</td><td>{}</td></tr>'.format(
            html.escape(row.get("metric", "")),
            html.escape(row.get("kernel_count", "")))
        for row in library_summary
    ]
    different_library_rows = _read_csv(
        ATEN_PAPER_ANALYSIS_DIR / "aten_different_library_families.csv")
    different_library_html = [
        '<tr><td><code>{}</code></td><td>{}<br><code>{}</code></td>'
        '<td>{}<br><code>{}</code></td></tr>'.format(
            html.escape(row.get("kernel", "")),
            html.escape(row.get("native_libraries", "")),
            html.escape(row.get("native_symbol", "")),
            html.escape(row.get("raised_library", "")),
            html.escape(row.get("raised_call", "")))
        for row in different_library_rows
    ]

    framework_winners = []
    for row in rows:
        if not row.get("comparability", "").startswith("FRAMEWORK_LEVEL"):
            continue
        if not (_aten_campaign_phase_ok(row, "native_gpu") and
                _aten_campaign_phase_ok(row, "raised")):
            continue
        native_us = float(_aten_campaign_value(row, "native_gpu"))
        raised_us = float(_aten_campaign_value(row, "raised"))
        if raised_us < native_us:
            framework_winners.append(
                f'<code>{html.escape(row["kernel"])}</code> '
                f'({native_us / raised_us:.2f}&times; raw)')
    ...
    for family, members in sorted(
        ...
        family_rows.append(
            ...
            f'<td>{sum(_aten_campaign_phase_ok(r, "cpu") and _aten_campaign_phase_ok(r, "native_gpu") and _aten_campaign_phase_ok(r, "raised") for r in members)}</td>'
            '</tr>')
    ...
    return (
        ...
        f'<div class="intro"><b>Active publication cohort: {len(rows)} kernels.</b> '
        ...
        'ATen coverage accounting</h3></div>'
        '<div class="intro">Library status and raised-IR structure are separate '
        'classifications. Complete, partial, and unmatched library rows sum to '
        'the 598-fixture corpus; fully Linalg, mixed, and no-Linalg rows form a '
        'second independent partition of the same corpus. '
        f'<a href="aten_paper_analysis_artifacts/aten_coverage_summary.csv">Source CSV</a> '
        f'· <a href="aten_paper_analysis_artifacts/aten_coverage_summary.tex">LaTeX table</a>'
        '</div><div class="table-wrap"><table class="audit-table paper-family">'
        '<thead><tr><th>classification</th><th>measurement</th><th>kernels</th>'
        ...
        f'<div class="audit-metric"><b>{three_way}</b><span>complete CPU/native/raised triples</span></div>'
        f'<div class="audit-metric"><b>{static_library}</b><span>complete static library mappings in full corpus</span></div>'
        '</div>'
        f'<div class="section-header"><h3 class="section-title">Measured GPU coverage within the {len(rows)}-kernel cohort</h3></div>'
        ...
        'their correctness gates at the same shape and dtype.</div>'
        '<div class="section-header"><h3 class="section-title">'
        'ATen: native and raised implementations</h3></div>'
        f'<div class="intro"><b>{len(measurements)} legally comparable, '
        f'correctness-gated GPU pairs.</b> {framework_pairs} additional '
        'native/raised pairs are retained as framework-level measurements and '
        'excluded from speedup claims because the public ATen COO path performs '
        'work outside the extracted C region. '
        'ATen is presented as a table because coverage, library mapping, and '
        'missing implementations are the primary result. Positive percentages '
        'favor the raised implementation; negative percentages are regressions. '
        f'The median raised/native ratio is {median_ratio:.3f}&times;, and the '
        f'median CPU/raised-GPU speedup is {median_cpu_speedup:.3f}&times;.</div>'
        '<div class="table-wrap"><table class="audit-table paper-plot-data"><thead><tr><th>rank</th>'
        '<th>kernel</th><th>family</th><th>shape</th><th>dtype</th>'
        '<th>Orin CPU, 1 core (µs, min of 5)</th>'
        '<th>ATen CUDA (µs, min of 5)</th>'
        '<th>raised resident (µs, min of 5)</th>'
        '<th>native GPU vs raised GPU (% ↑)</th>'
        '<th>native CPU vs raised GPU (% ↑)</th></tr></thead><tbody>'
        ...
        'of native CUDA. Rows between 1.1&times; and 1.6&times; are retained as '
        'competitive mappings, not reported as speedups. '
        'The explanation '
        'column is a hypothesis derived from the matched operation, selected '
        'external-library route, shape, and timing boundary—not causal profiling.</div>'
        + competitive_figure +
        '<div class="table-wrap"><table class="audit-table paper-plot-data">'
        '<thead><tr><th>kernel</th><th>ATen CUDA (µs, min of 5)</th>'
        '<th>raised GPU (µs, min of 5)</th><th>raised/native</th><th>selected route</th>'
        '<th>likely reason</th><th>comparison scope</th><th>interpretation</th>'
        '</tr></thead><tbody>' + "\n".join(competitive_rows) + '</tbody></table></div>'
        + (f'<div class="intro paper-provisional"><b>Excluded raw wins:</b> '
           + ', '.join(framework_winners)
           + '. These public ATen paths perform COO/container work outside the '
             'extracted C region, so their apparent speedups are boundary '
             'differences rather than legal same-region claims.</div>'
           if framework_winners else '')
        + '<div class="section-header"><h3 class="section-title">'
          'Native ATen versus Polygeist library discovery</h3></div>'
          '<div class="intro">Counts cover the common 598-fixture audit universe. '
          'A native external-library path may be a linked vendor call or a '
          'template-generated CUB, Thrust, or CUTLASS implementation. A '
          'Polygeist finding counts only a complete external-library rewrite. '
          'Conditional native paths are source-supported possibilities and do '
          'not assert that every shape selects that backend.</div>'
          '<div class="paper-data-grid"><div class="table-wrap">'
          '<table class="audit-table paper-family"><thead><tr>'
          '<th>classification</th><th>kernels</th></tr></thead><tbody>'
          + "\n".join(library_summary_rows) + '</tbody></table></div>'
          '<div class="paper-notes"><div><b>LaTeX artifact</b><span>'
          f'<a href="{paper_artifacts}/aten_native_library_comparison.tex">'
          'Download the paper table</a></span></div><div><b>Source data</b><span>'
          f'<a href="{paper_artifacts}/aten_native_library_comparison.csv">'
          'Aggregate CSV</a></span></div></div></div>'
          '<h4 style="margin-left:20px">Different library families selected '
          'for the same computation</h4><div class="table-wrap">'
          '<table class="audit-table paper-family"><thead><tr><th>kernel</th>'
          '<th>native ATen route</th><th>Polygeist route</th></tr></thead><tbody>'
          + "\n".join(different_library_html) + '</tbody></table></div>'
        ...
        '<th>raised GPU measured</th><th>GPU pairs</th><th>complete triples</th>'
        ...
        f'<div><b>{framework_pairs} framework-level native GPU cases</b><span>'
        '<code>aten_hspmm_cpu</code> and <code>aten_sparse_addmm_cpu</code> '
        'include extra ATen COO handling and are not used for exact-region '
        'speedup claims.</span></div></div>'
    )
...
def _aten_section(aten_stats: dict[str, dict], kernels: list[str]) -> str:
    ...
    for kernel in kernels:
        ...
        if native_us:
            ...
            _native_scope = html.escape(
                campaign.get("comparability") or
                _np.get("comparability", ""))
            _native_api = html.escape(_np.get("benchmark_api", "torch"))
            native_cell = (
                f'<td style="color:{_native_color};font-weight:600" '
                f'title="ATen native at the resident shape {_nat_shape} '
    ...
    headers = [
        ...
        'ATen CUDA resident, min of 5 (<span style="text-transform:none">µs</span>)',
        'PyTorch CPU x86-64, 24 threads (<span style="text-transform:none">µs</span>)',
        'ATen CPU Jetson Orin, 1 core, min of 5 (<span style="text-transform:none">µs</span>)',
    ...
    return (
        ...
        f'minimum-of-five results, {campaign_native} have correctness-gated ATen CUDA minima, '
        f'{campaign_raised} have correctness-gated raised resident minima, and '
        ...
        'minimum of those five samples as the headline result. '
...
def _b300_results_page() -> str:
    """Render retained B300 precision and related-work measurements."""
    raised64 = _read_csv(
        SECTION42_RESULTS_DIR / "b300_fp64_2026-09-09" / "summary.csv")
    raised32 = _read_csv(
        SECTION42_RESULTS_DIR / "b300_fp32_2026-09-09" / "summary.csv")
    cpu64 = _read_csv(
        SECTION42_RESULTS_DIR / "b300_fp64_2026-09-09" /
        "native_cpu_summary.csv")
    cpu32 = _read_csv(
        SECTION42_RESULTS_DIR / "b300_fp32_2026-09-09" /
        "native_cpu_summary.csv")
    native32 = _read_csv(
        SECTION42_RESULTS_DIR / "b300_polybenchgpu_fp32_2026-09-09" /
        "summary.csv")
    related = _read_csv(
        SECTION42_RESULTS_DIR / "b300_related_work_2026-09-09.csv")
    x86_related = _read_csv(
        SECTION42_RESULTS_DIR / "related_work_x86_2026-09-09.csv")
    ...
    r64, r32 = keyed(raised64), keyed(raised32)
    c64, c32 = keyed(cpu64), keyed(cpu32)
    n32 = keyed(native32)
    native64_rows = [
        row for row in related
        if row.get("suite") == "PolyBenchGPU" and row.get("dtype") == "fp64"
    ]
    n64 = keyed(native64_rows)

    def status_label(status: str) -> str:
        raw = status or "unavailable"
        css = "pass" if raw == "pass" else (
            "blocked" if raw in {"unavailable", "unmatched", "build_failed"}
            else "fail")
        return (f'<span class="result-status {css}">'
                f'{html.escape(raw.replace("_", " "))}</span>')

    def timing_cell(row: dict[str, str] | None, field: str = "e2e_min_ms",
                    detail: str = "") -> str:
        if not row:
            return "&mdash;"
        status = row.get("status", "")
        value = row.get(field, "")
        timing = f'<b>{float(value):.3f} ms</b>' if value else "&mdash;"
        extra = f'<br><span class="scope">{html.escape(detail)}</span>' if detail else ""
        return f'{timing}<br>{status_label(status)}{extra}'

    precision_rows = []
    kernels = sorted(set(r64) | set(r32) | set(c64) | set(c32) | set(n64) | set(n32))
    for kernel in kernels:
        precision_rows.append(
            f'<tr><td><b>{html.escape(kernel)}</b></td>'
            f'<td>{timing_cell(c64.get(kernel), "min_ms", "CPU kernel scope")}</td>'
            f'<td>{timing_cell(r64.get(kernel), "e2e_min_ms", "raised function E2E")}</td>'
            f'<td>{timing_cell(n64.get(kernel), "min_ms", "native CUDA E2E")}</td>'
            f'<td>{timing_cell(c32.get(kernel), "min_ms", "CPU kernel scope")}</td>'
            f'<td>{timing_cell(r32.get(kernel), "e2e_min_ms", "raised function E2E")}</td>'
            f'<td>{timing_cell(n32.get(kernel), "e2e_min_ms", "native CUDA E2E")}</td>'
            '</tr>')

    other_b300 = [row for row in related if row.get("suite") != "PolyBenchGPU"]
    b300_rows = []
    for row in other_b300:
        distribution = "&mdash;"
        if row.get("min_ms"):
            distribution = f'<b>{float(row["min_ms"]):.3f} ms</b>'
            details = []
            for label, field in (("median", "median_ms"), ("max", "max_ms"),
                                 ("IQR", "iqr_ms")):
                if row.get(field):
                    details.append(f'{label} {float(row[field]):.3f}')
            if details:
                distribution += ('<br><span class="scope">' +
                                 '; '.join(details) + '</span>')
        b300_rows.append(
            f'<tr><td><b>{html.escape(row.get("suite", ""))}</b></td>'
            f'<td>{html.escape(row.get("kernel", ""))}</td>'
            f'<td>{html.escape(row.get("dtype", "").upper())}</td>'
            f'<td>{html.escape(row.get("dataset", ""))}</td>'
            f'<td>{html.escape(row.get("variant", ""))}</td>'
            f'<td>{status_label(row.get("status", ""))}</td>'
            f'<td>{distribution}</td>'
            f'<td class="scope-cell">{html.escape(row.get("scope", ""))}</td></tr>')

    x86_rows = []
    for row in x86_related:
        x86_rows.append(
            f'<tr><td><b>{html.escape(row["suite"])}</b></td>'
            f'<td>{html.escape(row["kernel"])}</td>'
            f'<td>{html.escape(row["dtype"].upper())}</td>'
            f'<td>{html.escape(row["dataset"])}</td>'
            f'<td>{status_label(row["status"])}</td>'
            f'<td><b>{float(row["min_ms"]):.3f} ms</b></td>'
            f'<td class="scope-cell">{html.escape(row["notes"])}</td></tr>')

    raised_pass = sum(row.get("status") == "pass" for row in raised64 + raised32)
    native_pass = sum(row.get("status") == "pass"
                      for row in native64_rows + native32)
    related_pass = sum(row.get("status", "").lower() == "pass"
                       for row in other_b300)
    return (
        '<div class="section-header"><h2 class="section-title">B300 benchmark archive</h2></div>'
        '<div class="intro paper-provisional"><b>Secondary hardware results.</b> '
        'All measured values are retained here, including failures, but GPU clocks '
        'could not be locked in the container. FP32 and FP64 are never pooled. '
        'Different datasets, hardware, and timing scopes are displayed explicitly; '
        'the viewer does not manufacture ratios between incompatible rows.</div>'
        '<div class="audit-metrics">'
        f'<div class="audit-metric"><b>{raised_pass}/30</b><span>Polygeist FP32/FP64 passes</span></div>'
        f'<div class="audit-metric"><b>{native_pass}/28</b><span>native PolyBenchGPU passes</span></div>'
        f'<div class="audit-metric"><b>{related_pass}/{len(other_b300)}</b><span>other-paper B300 rows passing</span></div>'
        f'<div class="audit-metric"><b>{len(x86_related)}</b><span>separate x86 related-work rows</span></div>'
        '</div>'
        '<div class="section-header"><h3 class="section-title">PolyBench precision results</h3></div>'
        '<div class="intro">Minimum of five measurements after five warmups. CPU '
        'kernel scope and GPU E2E scopes are labelled in every cell. Native '
        'PolyBenchGPU rows use modified-source normalization adapters and preserve '
        'the upstream computational kernels. Gram-Schmidt failures remain visible '
        'and are excluded from performance claims.</div>'
        '<div class="table-wrap"><table class="audit-table b300-table"><thead><tr>'
        '<th>kernel</th><th>native CPU FP64</th><th>Polygeist FP64</th>'
        '<th>PolyBenchGPU FP64</th><th>native CPU FP32</th>'
        '<th>Polygeist FP32</th><th>PolyBenchGPU FP32</th>'
        '</tr></thead><tbody>' + ''.join(precision_rows) + '</tbody></table></div>'
        '<div class="section-header"><h3 class="section-title">Other-paper B300 reproductions</h3></div>'
        '<div class="intro">Every retained PPCG FP64/FP32, PolyBench-ACC FP32, '
        'and Ginsbach exact-region measurement is shown. PolyBench-ACC uses '
        'STANDARD rather than LARGE, and Ginsbach rows are extracted computational '
        'regions rather than the unrecovered whole author pipeline.</div>'
        '<div class="table-wrap"><table class="audit-table b300-table"><thead><tr>'
        '<th>paper/artifact</th><th>kernel or region</th><th>dtype</th><th>dataset/shape</th>'
        '<th>variant</th><th>status</th><th>runtime distribution</th><th>scope</th>'
        '</tr></thead><tbody>' + ''.join(b300_rows) + '</tbody></table></div>'
        '<div class="section-header"><h3 class="section-title">Related-paper CPU reproductions</h3></div>'
        '<div class="intro">These KernelFaRer, Pluto, and Polly measurements are '
        'from the separate x86 FP32/LARGE campaign. They are stored for completeness '
        'and are not compared numerically with B300 rows.</div>'
        '<div class="table-wrap"><table class="audit-table b300-table"><thead><tr>'
        '<th>paper/tool</th><th>kernel</th><th>dtype</th><th>dataset</th><th>status</th>'
        '<th>minimum</th><th>claim boundary</th></tr></thead><tbody>' +
        ''.join(x86_rows) + '</tbody></table></div>'
        '<div class="paper-notes"><div><b>Raw data</b><span>' + ' &middot; '.join((
            artifact("b300_fp64_2026-09-09/summary.csv", "Polygeist FP64 CSV"),
            artifact("b300_fp32_2026-09-09/summary.csv", "Polygeist FP32 CSV"),
            artifact("b300_polybenchgpu_fp32_2026-09-09/summary.csv", "native FP32 CSV"),
            artifact("b300_related_work_2026-09-09.csv", "B300 related-work CSV"),
            artifact("related_work_x86_2026-09-09.csv", "x86 related-work CSV"),
        )) + '</span></div><div><b>Protocol boundary</b><span>Headline values '
        'are minima. Median, maximum, and IQR are shown wherever retained. '
        'Correctness failures are not performance results.</span></div></div>')
...
def _polybench_paper_analysis_page() -> str:
    ...
    mlirsynth_rows = _read_csv(
        analysis_dir / "mlirsynth_synthesis_reproduction.csv")
    ...
    def ms(value: str, iqr: str = "", median: str = "", maximum: str = "") -> str:
        if not value:
            return "&mdash;"
        rendered = f"{float(value):.3f} ms"
        details = []
        if median:
            details.append(f"median {float(median):.3f}")
        if maximum:
            details.append(f"max {float(maximum):.3f}")
        if iqr:
            details.append(f"IQR {float(iqr):.3f}")
        if details:
            rendered += (" <span class=\"scope\">(" + "; ".join(details)
                         + ")</span>")
        return rendered
        ...
        cpu_html.append(
            ...
            '<td>{evidence}</td></tr>'.format(
                ...
                native=ms(row["native_ms"], row["native_iqr_ms"],
                          row.get("native_median_ms", ""), row["native_max_ms"]),
                raised=ms(row["raised_openblas_ms"], row["raised_iqr_ms"],
                          row.get("raised_median_ms", ""), row["raised_max_ms"]),
        ...
        gpu_html.append(
            ...
            '<td>{native_ok} / {raised_ok}</td><td>{modified}</td><td>{evidence}</td></tr>'.format(
                ...
                native_device=ms(row["native_device_ms"], row["native_device_iqr_ms"],
                                 row.get("native_device_median_ms", ""),
                                 row.get("native_device_max_ms", "")),
                native_e2e=ms(row["native_e2e_ms"]),
                raised_device=ms(row["raised_device_ms"], row["raised_device_iqr_ms"],
                                 row.get("raised_device_median_ms", ""),
                                 row.get("raised_device_max_ms", "")),
    ...
    mlirsynth_html = []
    for row in mlirsynth_rows:
        status = row.get("status", "unavailable")
        status_class = "pass" if status == "pass" else "blocked"
        seconds = (f'{float(row["wall_seconds"]):.3f} s'
                   if row.get("wall_seconds") else "&mdash;")
        ops = html.escape(row.get("raised_ops", "")) or "&mdash;"
        num_ops = html.escape(row.get("num_ops", "")) or "&mdash;"
        notes = html.escape(row.get("notes", "")) or "&mdash;"
        mlirsynth_html.append(
            f'<tr><td><b>{html.escape(row["kernel"])}</b></td>'
            f'<td><span class="result-status {status_class}">'
            f'{html.escape(status.replace("_", " "))}</span></td>'
            f'<td>{seconds}</td><td><code>{ops}</code></td>'
            f'<td>{num_ops}</td><td>{notes}</td></tr>')
    mlirsynth_passes = sum(
        row.get("status") == "pass" for row in mlirsynth_rows)
    ...
    def paper_pdf_figure(filename: str, title: str) -> str:
        url = f"{artifact_root}/paper_analysis/{filename}"
        return (
            f'<div class="paper-chart-wrap"><h4>{html.escape(title)}</h4>'
            f'<object data="{html.escape(url)}#view=FitH" '
            'type="application/pdf" width="100%" height="650" '
            f'aria-label="{html.escape(title)}">'
            f'<p><a href="{html.escape(url)}">Open {html.escape(title)} PDF</a>'
            '</p></object>'
            f'<div class="intro"><a href="{html.escape(url)}">Open the exact '
            'paper PDF</a></div></div>')

    def paper_svg_figure(filename: str, title: str, height: int = 720) -> str:
        url = f"{artifact_root}/paper_analysis/{filename}"
        return (
            f'<div class="paper-chart-wrap"><h4>{html.escape(title)}</h4>'
            f'<object data="{html.escape(url)}" type="image/svg+xml" '
            f'width="100%" height="{height}" aria-label="{html.escape(title)}">'
            f'<p><a href="{html.escape(url)}">Open {html.escape(title)}</a>'
            '</p></object>'
            f'<div class="intro"><a href="{html.escape(url)}">Open the full-size '
            'SVG</a></div></div>')

    paper_figures = (
        paper_svg_figure(
            "polybench_fp32_fp64_related_work_speedup.svg",
            "PolyBench: GPU and CPU speedup over native C",
            760)
        + paper_pdf_figure(
            "polybench_cpu_native_vs_raised.pdf",
    ...
    raised_only_rows = sorted(
        (row for row in main_gpu_rows
         if row.get("kernel") in {"cholesky", "symm", "trisolv", "trmm"}
         if row.get("raised_device_ms")
         and not row.get("native_device_ms")
         and row.get("raised_correctness") == "pass"),
        key=lambda row: float(row["raised_device_ms"]),
    )
    route_labels = {
        "cholesky": "cuSOLVER/cuBLAS",
        "symm": "cuBLAS symmetric composition",
        "trisolv": "cuBLAS DTRSV",
        "trmm": "cuBLAS DTRMV + DSCAL",
    }
    raised_only_svg = [
        '<div class="paper-chart-wrap"><h4>Raised-only GPU results: '
        'absolute device time and coverage</h4>',
        '<div class="paper-series-legend">'
        '<span><i style="background:#1a7f37"></i>raised GPU: correctness pass</span>'
        '<span><i style="background:#8c959f"></i>native GPU: unavailable</span>'
        '</div>',
        '<svg class="paper-chart" viewBox="0 0 1120 390" '
        'role="img" aria-label="Raised-only PolyBench GPU minimum-of-five device runtime">',
        '<rect x="18" y="18" width="745" height="330" rx="8" '
        'fill="#fafbfc" stroke="#d8dee8"/>',
        '<rect x="783" y="18" width="319" height="330" rx="8" '
        'fill="#fafbfc" stroke="#d8dee8"/>',
        '<text class="paper-axis-label" x="390" y="43" text-anchor="middle">'
        'Raised GPU device time (ms, logarithmic)</text>',
        '<text class="paper-axis-label" x="942" y="43" text-anchor="middle">'
        'Comparison availability</text>',
    ]
    log_min, log_max = -1.0, 3.0
    axis_left, axis_right = 170.0, 720.0
    for tick in (0.1, 1.0, 10.0, 100.0, 1000.0):
        x = axis_left + ((math.log10(tick) - log_min) /
                         (log_max - log_min)) * (axis_right - axis_left)
        raised_only_svg.append(
            f'<line class="paper-grid" x1="{x:.1f}" y1="58" '
            f'x2="{x:.1f}" y2="310"/>'
            f'<text class="paper-axis" x="{x:.1f}" y="331" '
            f'text-anchor="middle">{tick:g}</text>')
    for index, row in enumerate(raised_only_rows):
        kernel = row["kernel"]
        value = float(row["raised_device_ms"])
        x = axis_left + ((math.log10(value) - log_min) /
                         (log_max - log_min)) * (axis_right - axis_left)
        y = 78 + index * 61
        route = route_labels.get(kernel, "external GPU library")
        title = html.escape(
            f'{kernel} | raised GPU {value:.3f} ms | {route} | '
            'correctness pass | native GPU unavailable')
        raised_only_svg.extend((
            f'<text class="paper-axis" x="158" y="{y+16}" '
            f'text-anchor="end">{html.escape(kernel)}</text>',
            f'<rect x="{axis_left:.1f}" y="{y:.1f}" '
            f'width="{max(3.0, x-axis_left):.1f}" height="22" rx="3" '
            f'fill="#1a7f37"><title>{title}</title></rect>',
            f'<text class="paper-axis-label" x="{min(x+8, 726):.1f}" '
            f'y="{y+16}" text-anchor="start">{value:.3f}</text>',
            f'<text class="paper-axis" x="800" y="{y+13}">raised</text>',
            f'<rect x="852" y="{y-4}" width="72" height="23" rx="11" '
            f'fill="#dff5e5"/><text x="888" y="{y+12}" text-anchor="middle" '
            f'font-size="11" font-weight="bold" fill="#176b35">PASS</text>',
            f'<text class="paper-axis" x="800" y="{y+38}">native</text>',
            f'<rect x="852" y="{y+21}" width="72" height="23" rx="11" '
            f'fill="#e6e8eb"/><text x="888" y="{y+37}" text-anchor="middle" '
            f'font-size="11" font-weight="bold" fill="#57606a">N/A</text>',
            f'<text class="paper-axis" x="937" y="{y+13}">'
            f'{html.escape(route)}</text>',
        ))
    raised_only_svg.append(
        '<text class="paper-axis" x="20" y="375">Minimum of five synchronized '
        'CUDA-event measurements. Missing native values are not plotted as zero '
        'and do not contribute to speedup aggregates.</text></svg></div>')
    raised_only_chart = "".join(raised_only_svg)
    return (
        ...
        'one process, five warmups, five measured samples, and minimum-of-five '
        'headline reporting. The '
        ...
        + '<div class="section-header"><h3 class="section-title">mlirSynth '
          'reproduction</h3></div>'
          '<div class="audit-metrics">'
          f'<div class="audit-metric"><b>{mlirsynth_passes}</b><span>HLO '
          'synthesis cases reproduced locally</span></div>'
          '<div class="audit-metric"><b>20.8&times;</b><span>paper-reported Intel '
          'geomean; not reproduced locally</span></div></div>'
          '<div class="intro"><b>Why there is no mlirSynth runtime bar:</b> the '
          'released artifact provides FP64 synthesis/validation modules with '
          'small validation shapes, but not the paper&apos;s LARGE-FP32 XLA '
          'execution harness or raw per-kernel timing samples. The table below '
          'therefore reports the locally reproduced synthesis evidence. Adding '
          'the paper&apos;s 20.8&times; geomean to our same-machine per-kernel bars '
          'would mix hardware and measurement protocols.</div>'
          '<div class="table-wrap"><table class="audit-table"><thead><tr>'
          '<th>kernel</th><th>local synthesis status</th><th>wall time</th>'
          '<th>raised operations</th><th>operations in solution</th><th>notes</th>'
          '</tr></thead><tbody>' + "".join(mlirsynth_html) + '</tbody></table></div>'
        + '<div class="section-header"><h3 class="section-title">Comparison figures'
          '</h3></div>'
          '<div class="intro">The first figure adds the current B300 FP64/FP32 GPU '
          'comparisons and the newly measured x86 FP32 Polygeist CPU-raised '
          'series. The following CPU and GPU plots are the exact generated PDFs '
          'used by the paper. The CPU plot uses one continuous logarithmic '
        ...
        + '<div class="section-header"><h3 class="section-title">GPU results '
          ...
          '<div class="intro">These are successful compiler-generated external-'
          'library executions, shown using absolute raised-GPU device time. The '
          'gray native slot is intentionally marked unavailable, so the figure '
          'does not imply a speedup or add these kernels to GPU win/geomean counts.'
          '</div>'
        + raised_only_chart
        ...
        '<th>datatype</th>'
        '<th>native C -O3 (min of 5)</th><th>raised OpenBLAS (min of 5)</th><th>native / raised</th>'
        ...
        '<div class="table-wrap"><table class="audit-table"><thead><tr><th>kernel</th>'
        '<th>datatype</th>'
        '<th>native GPU device (min of 5)</th><th>raised GPU device (min of 5)</th><th>native / raised</th>'
        ...
        + " &middot; ".join((
            ...
            artifact("paper_analysis/polybench_cpu_native_vs_raised.pdf", "CPU figure PDF"),
            artifact("paper_analysis/polybench_gpu_native_vs_raised.pdf", "GPU figure PDF"),
            artifact("paper_analysis/polybench_fp32_fp64_related_work_speedup.svg",
                     "CPU/GPU speedup SVG"),
            artifact("paper_analysis/polybench_cpu_raised_fp32_x86_min5.csv",
                     "x86 raised-CPU samples"),
            artifact("paper_analysis/mlirsynth_synthesis_reproduction.csv",
                     "mlirSynth synthesis reproduction"),
...
def write_aten_paper_artifact_link() -> None:
    """Expose reproducible ATen paper CSV, TeX, and PDF artifacts."""
    artifact_link = OUTPUT_DIR / "aten_paper_analysis_artifacts"
    if artifact_link.is_symlink():
        if artifact_link.resolve() != ATEN_PAPER_ANALYSIS_DIR.resolve():
            artifact_link.unlink()
            artifact_link.symlink_to(
                ATEN_PAPER_ANALYSIS_DIR, target_is_directory=True)
    elif not artifact_link.exists():
        artifact_link.symlink_to(
            ATEN_PAPER_ANALYSIS_DIR, target_is_directory=True)
...
def _mfem_latest_paper_analysis_page(stats: list[dict]) -> str:
    ...
    pa_family_compile_rows = _read_csv(
        MFEM_C_ROOT / "pa_family_compile" / "results" / "summary.csv"
    )
    if not rows:
        return (
            '<div class="section-header"><h2 class="section-title">'
    ...
    provenance_labels = {
        "validated_standalone": "established standalone",
        "validated_qpoint_extract": "validated quadrature extract",
        "application_extract": "application-derived extract",
        "application_extract_hcurl_branch": "application-derived H(curl) branch",
        "compile_only_structural_extract": "new structural extract",
    }
    family_compile_rows = []
    abi_valid_families = 0
    abi_valid_launches = 0
    fully_raised_families = 0
    for row in pa_family_compile_rows:
        launches = int(row.get("kernel_launches", "0") or 0)
        residual_loops = int(row.get("residual_loops", "0") or 0)
        abi_valid = (launches > 0 and row.get("abi_lower_ok") == "true" and
                     int(row.get("abi_residual_launches", "0") or 0) == 0)
        if abi_valid:
            abi_valid_families += 1
            abi_valid_launches += launches
        if row.get("fully_raised") == "true":
            fully_raised_families += 1
        if abi_valid and residual_loops:
            status = ('<span class="partial">PARTIAL CAPTURE</span><br>'
                      '<small>residual loops remain</small>')
        elif abi_valid:
            status = '<span class="pass">CAPTURED</span>'
        elif launches:
            status = '<span class="fail">INVALID CAPTURE</span>'
        else:
            status = '<span class="none">UNMATCHED</span>'
        symbols = row.get("launch_symbols", "")
        target = (f'<code>{html.escape(symbols)}</code>' if symbols else 'None')
        provenance = provenance_labels.get(
            row.get("provenance", ""), row.get("provenance", "unknown")
        )
        family_compile_rows.append(
            '<tr>'
            f'<td><b>{html.escape(row.get("mfem_class", ""))}</b></td>'
            f'<td>{html.escape(provenance)}</td>'
            f'<td>{row.get("linalg_ops", "0")}</td>'
            f'<td>{residual_loops}</td><td>{target}</td>'
            f'<td>{launches if abi_valid else 0}</td><td>{status}</td></tr>'
        )
    family_compile_section = ''
    if pa_family_compile_rows:
        family_compile_section = (
            '<div class="section-header"><h3 class="section-title">Matched MFEM '
            'operators and library mappings — all 22 PA families</h3></div>'
            '<div class="intro">All <b>22/22</b> family fixtures pass the C '
            'frontend and reach Linalg; '
            f'<b>{fully_raised_families}/22</b> have no residual loops. '
            f'<b>{abi_valid_families}/22</b> contain at least one ABI-valid '
            f'external-library capture, totaling <b>{abi_valid_launches}</b> '
            'launches, with <b>0 invalid proposals</b>. A captured family may '
            'still contain unmatched work; PARTIAL CAPTURE identifies the '
            'families that retain explicit loops. The ten new structural '
            'extracts are compile evidence only and have not been numerically '
            'validated against MFEM.</div>'
            '<table class="audit-table paper-family"><caption>Compile-only '
            'raising and external-library capture across 22 PA families</caption>'
            '<thead><tr><th>MFEM family</th><th>fixture provenance</th>'
            '<th>Linalg ops</th><th>residual loops</th>'
            '<th>external-library target</th><th>ABI-valid launches</th>'
            '<th>compile status</th></tr></thead><tbody>'
            + ''.join(family_compile_rows) + '</tbody></table>'
        )
    operator_coverage_section = (
        family_compile_section +
        '<div class="section-header"><h3 class="section-title">Matched MFEM '
        'contraction sites with numerical validation</h3></div>'
...
def _llama_forward_runtime_summary() -> str:
    protocol = {
        row.get("run_id", ""): row for row in _read_csv(LLAMA_PROTOCOL_RESULTS)
    }

    def minimum(run_id: str) -> float:
        try:
            return float(protocol.get(run_id, {}).get("min_ms", "nan"))
        except ValueError:
            return math.nan

    native = minimum("protocol_native_cpu")
    raised_host = minimum("protocol_raised_gpu_host")
    raised_event = minimum("protocol_raised_gpu_device")
    expert = minimum("protocol_ggml_cuda")
    expert_speedup = native / expert
    return (
        '<div class="intro" style="padding-top:0">'
        ...
        f'<td>{native:.3f} ms</td><td><span class="pass">eligible reference</span></td>'
        '<td>numerical reference</td>'
        '<td>one process; minimum of 5 synchronized iterations after 5 warm-ups</td>'
        '</tr>'
        '<tr>'
        '<td><b>Polygeist raised Jetson GPU</b></td>'
        f'<td>{raised_host:.3f} ms synchronized host<br>{raised_event:.3f} ms CUDA event</td>'
        ...
        f'<td>{expert:.3f} ms</td><td><span class="pass">eligible expert baseline</span></td>'
        '<td>PASS: 32,000 logits at atol=1e-2, rtol=1e-4</td>'
        f'<td>{expert_speedup:.2f}&times; faster than native CPU; same 1&times;5+5 protocol</td>'
        '</tr>'
        '</tbody></table>'
...
def _llama_paper_analysis_page() -> str:
    ...
    native_ms = number(native, "min_ms")
    ggml_ms = number(ggml, "min_ms")
    speedup = native_ms / ggml_ms if native_ms > 0 and ggml_ms > 0 else math.nan

    # Only correctness-eligible measurements enter the charts. A red X keeps
    # the requested comparison visible when the raised implementation is not
    # currently claimable.
    ...
    for row in rows:
        ...
        audit_rows.append(
            ...
            f'<td>{samples}</td>'
            f'<td><b>{number(row, "min_ms"):.6f}</b></td>'
            f'<td>{number(row, "median_ms"):.6f}</td>'
            f'<td>{number(row, "max_ms"):.6f}</td>'
    ...
    return (
        ...
        '<div class="paper-matrix">'
        f'<div><b>{native_ms:.3f} ms</b><span>vanilla CPU on Orin (min of 5)<br>'
        '<span class="pass">eligible numerical reference</span></span></div>'
        f'<div class="paper-provisional"><b>{number(raised_gpu_host, "min_ms"):.3f} ms</b><span>raised Jetson GPU, synchronized host wall (min of 5)<br>'
        '<span class="none">slot pending: strict correctness failed</span></span></div>'
        f'<div><b>{ggml_ms:.3f} ms</b><span>expert-written ggml CUDA (min of 5)<br>'
        ...
        '<div><b>minimum</b><span>headline minimum of five; retain median, max, quartiles, IQR, and every sample</span></div>'
        '</div>'
        '<div class="paper-notes">'
        ...
        '<table class="audit-table paper-exclusions"><thead><tr>'
        '<th>implementation / scope</th><th>five samples (ms)</th>'
        '<th>minimum (headline)</th><th>median</th><th>maximum</th>'
        '<th>Q1–Q3 / IQR</th><th>checksum</th>'
        '<th>correctness</th><th>paper status</th></tr></thead><tbody>'
        + ''.join(audit_rows) + '</tbody></table>'
        '<div class="section-header"><h3 class="section-title">'
        'Why the raised rows are excluded</h3></div>'
        '<div class="paper-notes">'
        '<div class="paper-provisional"><b>Raised CPU / NVPL</b><span>Minimum-of-five '
        f'{number(raised_cpu, "min_ms"):.3f} ms; 10,456/32,000 logits fail '
        ...
        '<div class="paper-provisional"><b>Raised resident GPU</b><span>Minimum-of-five '
        f'{number(raised_gpu_host, "min_ms"):.3f} ms host and '
        f'{number(raised_gpu_event, "min_ms"):.3f} ms event; 7,922/32,000 '
...
def build_site_pages(polybench_stats: dict[str, dict],
    ...
    def nav() -> str:
        return (
            ...
            '<a href="b300.html">B300 benchmark archive</a> &middot; '
    ...
    landing = (
        ...
        + card("b300.html", "B300 benchmark archive", 204,
               "FP32/FP64 Polygeist and native CUDA results plus PPCG, PolyBench-ACC, Ginsbach, KernelFaRer, Pluto, and Polly measurements.")
    ...
    pages = {
        ...
        "b300.html": render_html(
            "Polygeist: B300 benchmark archive", b300, extra_css
        ),
