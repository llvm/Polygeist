#!/bin/bash
# Cross-compile the external cuSPARSE JDS-adapter smoke for Jetson.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/common_env.sh"

OUT="${1:-/tmp/cusparse_jds_smoke_jetson}"
...
"$CC" -O2 -I"$RT" \
  -c "$REPO_ROOT/issues/ginsbach_asplos18/cusparse_jds_smoke.c" \
  -o "$WORK/smoke.o"
