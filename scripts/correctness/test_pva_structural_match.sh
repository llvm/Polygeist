...
# Only the two source-semantics-exact operations are executable selections.
production_match() {
  local stem="$1" expected="$2"
...
"$PYTHON" "$PRODUCTION_MATCHER" "$AUDIT_DIR/renamed.mlir" \
  --enable-structured-rewrite --dry-run 2>&1 | grep -q 'pvaBoxFilter' && {
    echo 'non-exact renamed box candidate selected unexpectedly' >&2
    exit 1
  }

echo 'PASS: 6 candidates; 3 exact production selections; safety tests passed'
