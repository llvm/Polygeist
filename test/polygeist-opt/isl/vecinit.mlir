...
// ISL_OUT: domain: "[P0] -> { S1[i0] : (i0) mod 7 = 0 and 0 <= i0 < P0 }"
// ISL_OUT: accesses:
// ISL_OUT:   - S1:
// ISL_OUT:       reads:
// ISL_OUT:       writes:
// ISL_OUT:         - "[P0] -> { [i0] -> A1[o0, o1] : o0 = 1 + i0 and o1 = 42 + 234P0 - 687i0 }"
// ISL_OUT: domain: "[P0] -> { S2[i0, i1] : (i0) mod 5 = 0 and 0 <= i0 < P0 and 0 <= i1 < P0 }"
// ISL_OUT: accesses:
// ISL_OUT:   - S2:
// ISL_OUT:       reads:
// ISL_OUT:       writes:
// ISL_OUT:         - "[P0] -> { [i0, i1] -> A1[o0] : o0 = 43 + i0 + i1 }"
// ISL_OUT: { domain: "[P0] -> { S0[i0] : 0 <= i0 < P0 }", child: { schedule: "[P0] -> L0[{ S0[i0] -> [(i0)] }]" } }
// ISL_OUT: { domain: "[P0] -> { S1[i0] : (i0) mod 7 = 0 and 0 <= i0 < P0 }", child: { schedule: "[P0] -> L0[{ S1[i0] -> [(i0)] }]" } }
// ISL_OUT: { domain: "[P0] -> { S2[i0, i1] : (i0) mod 5 = 0 and 0 <= i0 < P0 and 0 <= i1 < P0 }", child: { schedule: "[P0] -> L1[{ S2[i0, i1] -> [(i0)] }]", child: { schedule: "[P0] -> L0[{ S2[i0, i1] -> [(i1)] }]" } } }
