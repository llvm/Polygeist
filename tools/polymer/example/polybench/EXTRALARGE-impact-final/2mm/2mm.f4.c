...
static void kernel_2mm(unsigned int ni, unsigned int nj, unsigned int nk, unsigned int nl, DATA_TYPE alpha,
 ...
 unsigned int lb, ub, lbp, ubp, lb2, ub2;
 register unsigned int lbv, ubv;

/* Generated from 2mm.pluto.cloog by CLooG 0.20.0-UNKNOWN gmp bits in 0.15s. */
for (t2=0;t2<=floord(_PB_NI-1,32);t2++) {
  for (t3=0;t3<=floord(_PB_NJ+_PB_NL-101,32);t3++) {
