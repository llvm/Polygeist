...
static
void kernel_seidel_2d(unsigned int tsteps,
		      unsigned int n,
		      DATA_TYPE POLYBENCH_2D(A,N,N,n,n))
{
  unsigned int t, i, j;
...
if ((_PB_N >= 3) && (_PB_TSTEPS >= 1)) {
  for (t1=0;t1<=floord(2*_PB_TSTEPS+_PB_N-4,32);t1++) {
    for (t2=max(ceild(t1,2),ceild(32*t1-_PB_TSTEPS+1,32));t2<=min(min(floord(_PB_TSTEPS+_PB_N-3,32),floord(32*t1+_PB_N+29,64)),t1);t2++) {
