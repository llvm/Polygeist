...
if (_PB_N >= 2) {
  ...
  table[0][1] = max_score(table[0][1], table[0 +1][1]);;
  table[0][1] = max_score(table[0][1], table[0 +1][1 -1]);;
  if (_PB_N >= 3) {
    lbv=0;
    ubv=1;
#pragma ivdep
#pragma vector always
    for (t8=lbv;t8<=ubv;t8++) {
      table[t8][2] = max_score(table[t8][2], table[t8][2 -1]);;
    }
    ...
    table[1][2] = max_score(table[1][2], table[1 +1][2 -1]);;
...
#pragma vector always
    for (t8=lbv;t8<=ubv;t8++) {
      table[t8][t1] = max_score(table[t8][t1], table[t8][t1-1]);;
    }
...
#pragma vector always
    ...
    for (t3=0;t3<=t1-2;t3++) {
      for (t4=t3+1;t4<=t1-1;t4++) {
        table[t3][t1] = max_score(table[t3][t1], table[t3][t4] + table[t4+1][t1]);;
      }
    }
  }
}
