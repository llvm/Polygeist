...
for dir in $dirlist; do
	if [ -d $dir ]; then
		...
		# compile polymer
		mlir-opt -inline -memref-dataflow-opt -canonicalize -symbol-dce -lower-affine -convert-scf-to-std -convert-std-to-llvm -canonicalize $dir.pluto.mlir | mlir-translate -mlir-to-llvmir > $dir.polymer.ll
