...
void kernel_2mm(int ni, int nj, int nk, int nl,
		...
		DATA_TYPE POLYBENCH_2D(C,NJ,NL,nj,nl),
		DATA_TYPE POLYBENCH_2D(D,NI,NL,ni,nl),
    DATA_TYPE POLYBENCH_1D(S1,NK,nk),
    DATA_TYPE POLYBENCH_1D(S2,NJ,nj))
{
  int i, j, k;
      ...
      for (t3=0;t3<=floord(_PB_NJ-1,32);t3++) {
        if (_PB_NK >= 1) {
          for (t5=0;t5<=floord(_PB_NK-1,32);t5++) {
            for (t7=32*t5;t7<=min(_PB_NK-1,32*t5+31);t7++) {
              for (t8=32*t3;t8<=min(_PB_NJ-1,32*t3+31);t8++) {
                S1[t7] = alpha * A[t2][t7] * B[t7][t8];;
                tmp[t2][t8] += S1[t7];;
              }
            }
          }
...
int main(int argc, char** argv)
{
  ...
  POLYBENCH_1D_ARRAY_DECL(S1,DATA_TYPE,NK,nk);
  POLYBENCH_1D_ARRAY_DECL(S2,DATA_TYPE,NJ,nj);
  ...
  POLYBENCH_FREE_ARRAY(S1);
  POLYBENCH_FREE_ARRAY(S2);
