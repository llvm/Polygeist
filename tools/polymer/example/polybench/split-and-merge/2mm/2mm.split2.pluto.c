...
if (_PB_NI >= 1) {
  ...
  for (t2=0;t2<=floord(_PB_NJ-1,32);t2++) {
    for (t3=0;t3<=floord(_PB_NI-1,32);t3++) {
      if ((_PB_NK >= 1) && (_PB_NL >= 1)) {
        for (t4=32*t2;t4<=min(_PB_NJ-1,32*t2+31);t4++) {
          for (t5=32*t3;t5<=min(_PB_NI-1,32*t3+31);t5++) {
            for (t7=0;t7<=_PB_NK-1;t7++) {
              tmp[t5][t4] += alpha * A[t5][t7] * B[t7][t4];;
            }
            for (t7=0;t7<=_PB_NL-1;t7++) {
              S[t4] = tmp[t5][t4] * C[t4][t7];;
              D[t5][t7] += S[t4];;
            }
          }
        }
      }
      if ((_PB_NK >= 1) && (_PB_NL <= 0)) {
        for (t4=32*t2;t4<=min(_PB_NJ-1,32*t2+31);t4++) {
          for (t5=32*t3;t5<=min(_PB_NI-1,32*t3+31);t5++) {
            for (t7=0;t7<=_PB_NK-1;t7++) {
              tmp[t5][t4] += alpha * A[t5][t7] * B[t7][t4];;
