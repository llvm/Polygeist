  ...
  for (t2=0;t2<=_PB_N-1;t2++) {
    ...
    if (_PB_M >= 1) {
      for (t4=t2+1;t4<=_PB_N-1;t4++) {
        for (t7=0;t7<=_PB_M-1;t7++) {
          R[t2][t4] += Q[t7][t2] * A[t7][t4];;
        }
        for (t7=0;t7<=floord(_PB_M-1,32);t7++) {
          for (t8=32*t7;t8<=min(_PB_M-1,32*t7+31);t8++) {
            S[t8] = Q[t8][t2] * R[t2][t4];;
          }
        }
        for (t7=0;t7<=floord(_PB_M-1,32);t7++) {
          for (t8=32*t7;t8<=min(_PB_M-1,32*t7+31);t8++) {
            A[t8][t4] = A[t8][t4] - S[t8];;
          }
        }
      }
    }
